...
pub async fn get_reader_state(
    ...
    Ok(state.lock().unwrap().reader.clone())
}
...
#[tauri::command]
pub async fn get_settings(state: State<'_, Arc<Mutex<AppState>>>) -> Result<SettingStore, String> {
    Ok(state.lock().unwrap().setting.clone())
}

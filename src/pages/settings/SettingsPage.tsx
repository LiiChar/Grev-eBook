				...
				<main class='flex-1 overflow-y-auto p-4 md:p-6'>
					...
					<div class='max-w-3xl mx-auto space-y-6'>
						...
						<Show when={activeSection() === 'general'}>
							...
							<SettingsGroup title='База данных'>
								{/* Theme */}
								<SettingRow
									label='Очистить базу данных'
									description='Удалить все данные из базы данных'
								>
						...
						<Show when={activeSection() === 'ui'}>
							<SettingsGroup title='Настройки интерфейса'>
								...
								>
									<ToggleSwitch checked={settings.ui.auto_hide} onChange={setAutoHide} />
								</SettingRow>

								<SettingRow label='Анимации' description='Плавные переходы и анимации'>
									<ToggleSwitch
						...
						<Show when={activeSection() === 'hotkeys'}>
							<SettingsGroup title='Горячие клавиши'>
								<div class='p-4 space-y-3'>
									<Hotkey label='Следующая страница' keys={['→', 'Space', 'PgDn']} />
	...
	return (
		<SettingsGroup variant={variant} title='Настройки чтения'>
			...
			{/* <SettingRow
				variant={variant}
				label='Масштаб PDF'
				description={`${Math.round(settings.reader.pdf_zoom * 100)}%`}
			>
				<div class='flex items-center gap-3'>
					<GlassButton
						size='icon'
						variant='ghost'
						onClick={() => setPdfZoom(settings.reader.pdf_zoom - 0.1)}
					>
						<Icon name='minus' size={16} />
					</GlassButton>
					<input
						...
						max='3'
						step='0.1'
						value={settings.reader.pdf_zoom}
						onInput={e => setPdfZoom(parseFloat(e.currentTarget.value))}
						class='w-full'
					/>
					<GlassButton
						size='icon'
			...
			{/* <SettingRow
				variant={variant}
				label='Зафиксировать зум PDF'
				description={settings.reader.pdf_zoom_lock ? 'Включен' : 'Выключен'}
			>
				<ToggleSwitch
					checked={settings.reader.pdf_zoom_lock}
					onChange={setPdfZoomLock}
				/>
			</SettingRow> */}
		</SettingsGroup>
	);
